"""
LSPU Job Posting Web API
Flask endpoints for generating and managing LSPU job posting templates
"""
...
@job_posting_bp.route('/api/campus-locations', methods=['GET'])
def get_campus_locations():
    ...
    try:
        ...
        cursor.execute("SELECT id, campus_name, campus_code FROM campus_locations WHERE is_active = 1 ORDER BY campus_name")
        campuses = [{'id': row[0], 'name': row[1], 'code': row[2]} for row in cursor.fetchall()]
