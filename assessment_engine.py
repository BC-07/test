...
Implements the university-specific scoring algorithms based on Excel assessment criteria.

Scoring Categories:
- Education (40%): Relevance/Appropriateness, Basic Minimum, Doctoral Progress
- Experience (20%): Years of relevant professional experience
- Training (10%): Professional development and training hours
- Eligibility (10%): Professional licenses and certifications
- Accomplishments (5%): Awards, recognitions, and achievements
- Potential (15%): Interview and aptitude test scores (manual entry)
"""
...
class UniversityAssessmentEngine:
    ...
    def parse_lspu_job_requirements(self, lspu_job: Dict) -> Dict[str, Any]:
        """
        Parse LSPU job posting requirements into structured format for assessment
        
        Args:
            lspu_job: LSPU job posting dictionary with requirements fields
            
        Returns:
            Dict with parsed requirements compatible with assessment methods
        """
        requirements = {
    ...
    def assess_candidate_for_lspu_job(self, candidate_data: Dict, lspu_job: Dict, position_type_id: int = None) -> Dict[str, Any]:
        """
        Assess candidate against LSPU job posting requirements using university standards
        
        Args:
            candidate_data: Comprehensive candidate data including PDS information
            lspu_job: LSPU job posting with requirements fields
            position_type_id: Optional position type ID for templates
            
        Returns:
            Assessment results with detailed scoring breakdown
        """
        try:
    ...
    def _assess_education(self, pds_data: Dict, templates: List[Dict], 
        ...
        # Extract education data from PDS
        education_data = pds_data.get('education_data', pds_data.get('education', []))
        ...
        for degree in all_degrees:
            if isinstance(degree, dict):
                ...
                school = (degree.get('school') or '').strip()
        ...
        # Score Basic Minimum Requirement (35 points for Masteral)
        if highest_degree_level >= 6:  # Master's degree or higher
            details['basic_minimum_score'] = 35
            total_score += 35
        elif highest_degree_level >= 5:  # Bachelor's degree
            details['basic_minimum_score'] = 25  # Partial credit
            total_score += 25
        else:
    ...
    def _assess_training(self, pds_data: Dict, templates: List[Dict]) -> Tuple[float, Dict]:
        ...
        training_data = pds_data.get('training', pds_data.get('learning_development', []))
