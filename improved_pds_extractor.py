...
class ImprovedPDSExtractor:
    ...
    def _extract_educational_background(self, worksheet) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            # Look for the table structure starting a few rows after the header
            # Expected columns: LEVEL, NAME OF SCHOOL, BASIC EDUCATION/DEGREE/COURSE, 
            # PERIOD (FROM/TO), HIGHEST LEVEL/UNITS, YEAR GRADUATED, SCHOLARSHIP/HONORS
            
            table_start_row = education_start_row + 3  # Skip header rows
            education_levels = ['ELEMENTARY', 'SECONDARY', 'VOCATIONAL', 'COLLEGE', 'GRADUATE']
            
            for check_row in range(table_start_row, table_start_row + 10):
                for level in education_levels:
                    if self._find_level_row(worksheet, level, check_row, check_row + 10):
                        level_row = self._find_level_row(worksheet, level, check_row, check_row + 10)
    ...
    def _extract_education_entry(self, worksheet, row: int, level: str) -> Optional[Dict[str, Any]]:
        """Extract a single education entry from a row"""
        try:
            entry = {
                'level': level.lower(),
                'school': '',
            ...
            # Map values to entry fields based on expected positions
            non_empty_values = [v for v in values if v and v != level and v != 'None']
            ...
            # Only return entry if it has meaningful data
            if entry['school'] or entry['degree_course']:
                return entry
        ...
        try:
            ...
            for row in range(current_row, current_row + max_rows_to_check):
                ...
                if len(non_empty_values) >= 2:
                    ...
                    # Only add if it looks like actual eligibility data
                    if entry['eligibility'] and not entry['eligibility'].isdigit():
                    ...
                    for row in range(ref_start_row + start_offset, ref_start_row + start_offset + 10):
                        ...
                        if len(non_empty_values) >= 2:
                            ...
                            if self._looks_like_name(potential_name):
                                reference = {
                                    'name': potential_name,
                                    'address': non_empty_values[1] if len(non_empty_values) > 1 else '',
                                    'tel_no': non_empty_values[2] if len(non_empty_values) > 2 else ''
                                }
                                
                                # Look for phone number in any of the values
    ...
    def _extract_pdf_text(self, file_path: str) -> str:
        ...
        try:
            ...
            self.errors.append(f"Error reading PDF file: {str(e)}")
            return ""
    ...
    def _extract_educational_background_from_text(self, text: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            for i, level_data in enumerate(education_levels_data):
                ...
                if school_name or level_data.get('level'):
    ...
    def _extract_school_names_from_text(self, text: str) -> List[str]:
        """Extract school names from the NAME OF SCHOOL section"""
        school_names = []
        ...
        try:
            ...
            if edu_section_match:
                ...
                level_patterns = {
                    ...
                    'Graduate': r'GRADUATE\s+STUDIES\s+(\d{2}/\d{2}/\d{4})?\s+(\d{2}/\d{2}/\d{4})?\s+([\w\s/]+)?'
                }
                ...
                for level, pattern in level_patterns.items():
                    ...
                    for match in matches:
                        ...
                        level_entry = {
                            ...
                            'units_earned': groups[2] if len(groups) > 2 and groups[2] else None,
                ...
                degree_section = re.search(r'Bachelor\s+of\s+Science\s+in\s+Computer\s+Science', text, re.IGNORECASE)
                if degree_section and levels_data:
                    # Assign degree to college level
                    for entry in levels_data:
                        if entry['level'] == 'College':
                            entry['degree'] = degree_section.group().strip()
                            break
...
if __name__ == "__main__":
    ...
    # Test XLSX file
    xlsx_file = "Sample PDS Lenar.xlsx"
    if os.path.exists(xlsx_file):
        print("Testing XLSX extraction:")
        result = extractor.extract_pds_data(xlsx_file)
        print(f"XLSX extraction successful: {len(result) > 0}")
        if extractor.errors:
            print(f"XLSX errors: {extractor.errors}")
    ...
    if os.path.exists(pdf_file):
        ...
        print(f"PDF extraction successful: {len(result) > 0}")
        if result:
            print(f"Extracted sections: {list(result.keys())}")
            if 'personal_info' in result:
                personal = result['personal_info']
                print(f"Personal info fields: {len(personal)}")
                if 'email' in personal:
                    print(f"Email extracted: {personal['email']}")
        if extractor.errors:
            print(f"PDF errors: {extractor.errors}")
    else:
