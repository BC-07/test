...
class DatabaseManager:
    def __init__(self, db_url: str = None):
        ...
        if not self.db_url:
            ...
            if all([db_user, db_password, db_name]):
                # PostgreSQL is properly configured, use it
        ...
        if self.use_sqlite:
            ...
            with sqlite3.connect(sqlite_path) as conn:
                ...
                ''', (
                    ...
                    candidate_data.get('linkedin'),  # Optional for PDS
                    candidate_data.get('github'),    # Optional for PDS
                    candidate_data.get('resume_text', f"PDS extraction from {candidate_data.get('uploaded_filename', 'unknown')}"),  # Optional for PDS
                    ...
                    candidate_data.get('processing_type', 'pds'),  # Default to PDS
                    # PDS-specific fields
    ...
    def update_upload_session(self, session_id: str, **kwargs) -> bool:
        ...
        try:
            ...
            for key, value in kwargs.items():
                if key in ['status', 'file_count', 'completed_at', 'metadata', 'error_message', 'processed_count']:
                    if self.use_sqlite and key == 'metadata':
                        key = 'session_data'  # SQLite uses session_data
    ...
    def create_upload_file_record(self, session_id: str, file_id: str, file_data: Dict) -> bool:
        ...
        try:
            logger.info(f"Creating upload file record: session_id={session_id}, file_id={file_id}, file_data={file_data}")
            ...
            if self.use_sqlite:
                ...
                conn.commit()
                conn.close()
                logger.info(f"Successfully created SQLite upload file record for {file_id}")
                return True
            else:
                with self.get_connection() as conn:
                    cursor = conn.cursor()
                    
                    logger.info(f"Inserting PostgreSQL record: session_id={session_id}, filename={file_data.get('original_name')}, size={file_data.get('size')}, type={file_data.get('type')}")
                    ...
                    conn.commit()
                    logger.info(f"Successfully created PostgreSQL upload file record for {file_id}")
                return True
        except Exception as e:
            logger.error(f"Error creating upload file record for {file_id}: {e}")
            logger.error(f"File data was: {file_data}")
            return False
