...
class ResumeScreeningApp:
    def __init__(self):
        ...
        try:
            ...
            logger.warning(f"⚠️ Resume processor initialization failed: {e}")
            self.resume_processor = None
            
        try:
            self.pds_processor = PersonalDataSheetProcessor()
            logger.info("✅ PDS processor initialized successfully")
        except Exception as e:
            logger.warning(f"⚠️ PDS processor initialization failed: {e}")
            logger.info("ℹ️ PDS processing will use fallback methods")
        ...
        try:
            ...
            logger.info("✅ Clean upload handler initialized successfully")
        except Exception as e:
            logger.error(f"❌ Clean upload handler initialization failed: {e}")
            logger.error("❌ Upload system will not be available")
        ...
        try:
            ...
            logger.warning(f"⚠️ Assessment engine initialization failed: {e}")
            logger.info("ℹ️ Will use fallback assessment methods")
            self.assessment_engine = None
            
        self.processor = self.resume_processor  # Alias for compatibility
    ...
    def _register_routes(self):
        ...
        # Legacy upload endpoints (to be deprecated)
    ...
    def _process_file_for_analysis(self, file_record, job):
        ...
        try:
            ...
            logger.info(f"🔍 Processing {original_name} (type: {file_type})")
            ...
            if is_pds_file and file_type == 'excel':
                logger.info(f"📋 Processing as PDS: {original_name}")
                return self._process_pds_file(file_path, original_name, job)
            elif file_type == 'pdf':
                logger.info(f"📄 Processing as PDF resume: {original_name}")
                return self._process_pdf_file(file_path, original_name, job)
            else:
                logger.warning(f"⚠️ Unsupported file type for analysis: {file_type}")
                return None
                
        except Exception as e:
            logger.error(f"❌ Error processing file {file_record['original_name']}: {e}")
            return None
    ...
    def _process_pds_file(self, file_path, original_name, job):
        ...
        try:
            if self.pds_processor:
                # Use the enhanced PDS processor
                with open(file_path, 'rb') as file:
                    candidate_data = self.pds_processor.process_pds_candidate(file, job['id'])
                    ...
                    if candidate_data and 'pds_data' in candidate_data:
                        ...
                        if is_lspu_job and self.assessment_engine:
                            try:
                                logger.info(f"🎯 Using LSPU assessment engine for {original_name}")
                                ...
                                logger.info(f"✅ LSPU assessment completed: {percentage_score}%")
                            except Exception as e:
                                logger.warning(f"⚠️ LSPU assessment failed, using fallback: {e}")
                                score = self._calculate_pds_score(candidate_data, job)
                                percentage_score = score
                        else:
                            logger.info(f"📊 Using standard PDS scoring for {original_name}")
                            score = self._calculate_pds_score(candidate_data, job)
                            percentage_score = score
                ...
                logger.info(f"📊 Using fallback PDS processing for {original_name}")
                candidate_data = self._fallback_pds_processing(file_path, original_name, job)
                return candidate_data
                
        except Exception as e:
            logger.error(f"❌ Error processing PDS file {original_name}: {e}")
            return None
        ...
        try:
            if self.resume_processor:
                ...
                if text.strip():
                    ...
                    logger.warning(f"⚠️ No text extracted from PDF: {original_name}")
                    return None
            else:
                logger.error(f"❌ Resume processor not available for {original_name}")
                return None
                
        except Exception as e:
            logger.error(f"❌ Error processing PDF file {original_name}: {e}")
            return None
    ...
    def _fallback_pds_processing(self, file_path, original_name, job):
        ...
        try:
            ...
            try:
                ...
                logger.warning(f"⚠️ Error in basic extraction: {e}")
            ...
            logger.info(f"📋 Fallback PDS processing complete for {original_name}: {candidate_data['name']}")
            return candidate_data
            
        except Exception as e:
            logger.error(f"❌ Fallback PDS processing failed for {original_name}: {e}")
            return None
                ...
                if file.filename != '' and self._is_allowed_file(file.filename):
                    try:
                        ...
                        if self._is_image_file(file.filename):
                            ...
                            logger.info(f"Detected image file, using OCR processing: {file.filename}")
                        else:
                            processing_type = 'resume'
                        ...
                        # Store candidate data with comprehensive information
                        candidate_data = {
                            'name': result.get('name', 'Unknown'),
                            'email': result.get('email', ''),
                            'phone': result.get('phone', ''),
    ...
    def _process_resume_for_job(self, resume_text, filename, job):
        """Process a resume against a specific job with comprehensive extraction"""
        ...
        work_experience = self.resume_processor.extract_work_experience(resume_text)
        skills_info = self.resume_processor.extract_skills(resume_text)
        certifications = self.resume_processor.extract_certifications(resume_text)
        
        # Determine university position type using assessment system instead of ML prediction
        university_position = self._determine_university_position_type(
            education_info, work_experience, skills_info, certifications
        )
        
        # Get job requirements
        job_requirements = [skill.strip().lower() for skill in job['requirements'].split(',') if skill.strip()]
        
        # Calculate comprehensive scoring
        scoring_result = self._calculate_comprehensive_score(
            education_info, work_experience, skills_info, 
            certifications, job_requirements, job
        )
        ...
        return {
            ...
            'allSkills': skills_info,
            'certifications': certifications,
            'predictedCategory': university_position,  # Now using university position type
            'matchScore': scoring_result['total_score'],
            'matchedSkills': matched_skills,
            'missingSkills': missing_skills,
            'scoringBreakdown': scoring_result['breakdown'],
            'yearsOfExperience': self._calculate_years_of_experience(work_experience),
            'educationLevel': self._determine_education_level(education_info)
        }
    ...
    def _predict_job_category(self, resume_text):
        """DEPRECATED: Legacy ML prediction method - replaced by university position type system"""
        logger.warning("_predict_job_category is deprecated - use _determine_university_position_type instead")
        return self._determine_university_position_type_from_text(resume_text)
    ...
    def _determine_university_position_type_from_text(self, resume_text):
        """Helper method to determine position type from raw text (for legacy compatibility)"""
        try:
            # Extract basic info for position determination
            education_info = self.resume_processor.extract_education(resume_text)
            work_experience = self.resume_processor.extract_work_experience(resume_text)
            skills_info = self.resume_processor.extract_skills(resume_text)
            certifications = self.resume_processor.extract_certifications(resume_text)
            
            return self._determine_university_position_type(
                education_info, work_experience, skills_info, certifications
            )
        except Exception as e:
            logger.error(f"Error in position type determination from text: {e}")
            return {'category': 'Job Order', 'confidence': 50}
    ...
    def upload_pds(self):
        ...
        try:
            ...
            try:
                ...
                row = cursor.fetchone()
                if row:
                    logger.info(f"✓ Found LSPU job posting: ID={row[0]}, Title={row[1]}")
                    job = {
                        'id': row[0],
                        'title': row[1],
                        'category': row[2],
            ...
            logger.info(f"✓ Successfully loaded job: {job.get('title')} (ID: {job.get('id')})")
                    ...
                    try:
                        ...
                        if self._is_image_file(file.filename):
                            ...
                            if is_pds:
                                ...
                                candidate_data = self.processor.process_pds_candidate(file, job_id)
                                ...
                                logger.info(f"✓ Set job_id={job_id} for candidate: {candidate_data.get('name', 'Unknown')}")
                                ...
                                # Store in database
                                candidate_id = db_manager.create_candidate(candidate_data)
                                logger.info(f"✓ Created candidate with ID: {candidate_id}, job_id: {candidate_data['job_id']}")
        ...
        try:
            ...
            for file in files:
                ...
                try:
                    ...
                    # Extract PDS data using PDS processor
                    file.seek(0)
                    if self.pds_processor:
                        candidate_data = self.pds_processor.process_pds_candidate(file, job_id)
                    else:
                        # Fallback processing
                        candidate_data = {
                    ...
                    if is_lspu_job and self.assessment_engine:
                        try:
                            ...
                            logger.info(f"🎯 Starting LSPU university assessment for {file.filename} with job {job_id}")
                            assessment_result = self.assessment_engine.assess_candidate_for_lspu_job(
                            ...
                            percentage_score = assessment_result.get('percentage_score', 0)
                            scoring_breakdown = assessment_result.get('assessment_results', {})
                            logger.info(f"✅ LSPU university assessment completed for {file.filename} - Score: {percentage_score}%")
                            
                        except Exception as e:
                            logger.error(f"❌ LSPU assessment failed for {file.filename}: {e}")
                            logger.warning(f"🔄 Using fallback scoring for {file.filename}")
    ...
    def upload_files_clean(self):
        ...
        try:
            ...
            logger.info(f"📤 Upload request: {len(files)} files for job {job_id}")
            ...
            logger.info(f"📤 Processing upload for job: {job.get('title', 'Unknown')} (ID: {job_id})")
            ...
            logger.info(f"✅ Upload session {session_id} created with {successful_files} files for job {job_id}")
            ...
            logger.info(f"✅ Clean upload successful: {successful_files} files ready for analysis")
            return jsonify(response_data)
    ...
    def start_analysis(self):
        ...
        try:
            ...
            logger.info(f"🔍 Start analysis request: session_id={session_id}")
            ...
            logger.info(f"🔍 Starting analysis for {len(upload_files)} files in session {session_id}")
                ...
                try:
                    ...
                    if candidate_data:
                        ...
                        if candidate_id:
                            ...
                            logger.info(f"✅ Successfully processed: {file_record['original_name']} -> Candidate ID: {candidate_id}")
                        else:
            ...
            logger.info(f"✅ Analysis completed for session {session_id}: {successful_analyses}/{len(upload_files)} files processed")
            return jsonify(response_data)
            ...
            if session_id not in session['upload_sessions']:
                logger.error(f"❌ Session {session_id} not found in available sessions: {list(session['upload_sessions'].keys())}")
                return jsonify({'success': False, 'error': 'Upload session not found'}), 404
            ...
            if session_data.get('status') == 'completed':
                logger.info(f"🔄 Session {session_id} already completed, returning previous results")
                return jsonify({
                    'success': True, 
            ...
            logger.info(f"🔍 Session data: job_id={job_id}, file_count={len(file_data)}")
            logger.info(f"Starting analysis for session {session_id}: {len(file_data)} files")
            
            # Get job details
            job = self._get_job_details(job_id)
            logger.info(f"🔍 Job lookup result for ID {job_id}: {job is not None}")
            if not job:
    ...
    def _get_job_details(self, job_id):
        ...
        logger.info(f"🔍 Looking up job details for ID: {job_id} (type: {type(job_id)})")
        ...
        try:
            ...
            row = cursor.fetchone()
            logger.info(f"🔍 LSPU job query result: {row is not None}")
    ...
    def _process_excel_file(self, file_path, filename, job):
        ...
        try:
            ...
            if self.pds_processor:
                logger.info(f"🔍 Processing Excel PDS file: {filename}")
                return self.pds_processor.process_pdf_file(file_path, filename, job)
            else:
