...
class JobPostingManager {
    ...
    async loadCampusLocations() {
        try {
            ...
            if (data.success) {
                this.campusLocations = data.campuses;
    ...
    populateCampusSelect() {
        const select = document.getElementById('campusLocation');
        if (!select) return;
        
        select.innerHTML = '<option value="">Select Campus</option>';
        
        this.campusLocations.forEach(campus => {
                ...
                <div class="position-card card">
                    ...
                    <div class="card-body">
                        <div class="row">
                            <div class="col-md-8">
                                <div class="mb-2">
                                    <strong>Campus:</strong> ${posting.campus || 'Not specified'}
                                </div>
                                <div class="mb-2">
    ...
    validateForm() {
        ...
        const requiredFields = [
            ...
            'campus_id', 'application_deadline', 'education_requirements'
        ];
    ...
    getFormData() {
        ...
        if (data.quantity_needed) data.quantity_needed = parseInt(data.quantity_needed);
        if (data.salary_grade) data.salary_grade = parseInt(data.salary_grade);
        if (data.salary_amount) data.salary_amount = parseFloat(data.salary_amount);
        if (data.position_type_id) data.position_type_id = parseInt(data.position_type_id);
    ...
    generatePreviewHtml(data) {
        ...
        const campus = this.campusLocations.find(c => c.id == data.campus_id)?.name || 'Unknown Campus';
        ...
        return `
            <div class="preview-card border rounded p-3">
                <div class="text-center mb-3">
                    ...
                    <div class="text-primary mt-1">${campus}</div>
    ...
    populateFormWithJobData(jobData) {
        ...
        setFieldValue('campusLocation', jobData.campus_id);
